...
/**
 * VerticalButtonFieldSet aligns a group of buttons vertically so that they have
 * equal widths
 * 
 * This Manager does not respect Horizontal style bits on the child nodes since
 * they're all fully justified
 * 
 * If USE_ALL_WIDTH is passed into this manager then the children will also use
 * all available width
 */
public class HorizontalButtonFieldSet extends Manager {
	...
	protected void sublayout(int width, int height) {
		...
		for (int i = 0; i < numFields; i++) {
			...
			int leftMargin = Math.max(prevRightMargin,
					currentField.getMarginLeft()) / 2;
			int rightMargin = Math.max(nextLeftMargin,
					currentField.getMarginRight()) / 2;
			if (i == 0) {
			...
			int currentPreferredWidth = currentField.getPreferredWidth()
					+ FieldDimensionUtilities.getBorderWidth(currentField);
